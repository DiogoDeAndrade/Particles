...
HRESULT DXUtil_GetDXSDKMediaPathCch( TCHAR* strDest, int cchDest )
{
    ...
    if( lstrlen(strDest) + lstrlen(strMedia) < cchDest )
        _tcscat_s( strDest, 256, strMedia );
    else
        return E_INVALIDARG;
...
HRESULT DXUtil_FindMediaFileCch( TCHAR* strDestPath, int cchDest, TCHAR* strFilename )
{
    ...
    if( INVALID_HANDLE_VALUE != file )
    {
        _tcsncpy_s( strDestPath, 512,strShortName, cchDest );
    ...
    _tcsncpy_s( strDestPath, 8192, strFilename, cchDest );
    strDestPath[cchDest-1] = 0; // _tcsncpy doesn't NULL term if it runs out of space
                                    ...
                                    DWORD cchDest, TCHAR* strDefault )
{
    ...
    if( ERROR_SUCCESS != RegQueryValueEx( hKey, strRegName, 0, &dwType, 
                                          (BYTE*)strDest, &cbDest ) )
    {
        _tcsncpy_s( strDest, 8192, strDefault, cchDest );
        strDest[cchDest-1] = 0;
...
#else
    strncpy_s( strDestination, 8192, tstrSource, cchDestChar );
    strDestination[cchDestChar-1] = '\0';
    return S_OK;
#endif   
}
...
#else
    strncpy_s( tstrDestination, 8192, strSource, cchDestChar );
...
VOID DXUtil_LaunchReadme( HWND hWnd, TCHAR* strLoc )
{
    ...
    if( strLastSlash )
    {
        _tcsncpy_s( strExeName, 8192, &strLastSlash[1], MAX_PATH );
    ...
    if( !bFound )
    {
        ...
        _tcscpy_s( strReadmePath, 8192, strExePath );
...
{
    int nResult = _sntprintf_s( strDest, 8192, cchDestChar, TEXT("{%0.8X-%0.4X-%0.4X-%0.2X%0.2X-%0.2X%0.2X%0.2X%0.2X%0.2X%0.2X}"),
